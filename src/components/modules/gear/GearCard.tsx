...
export function GearCard({ gear }: GearCardProps) {
  ...
  return (
    <Card className="group flex flex-col h-full border-slate-800/80 bg-slate-900/70 hover:border-emerald-500/50 hover:shadow-emerald-500/10 transition-all duration-300 overflow-hidden">
      ...
      <div className="relative w-full h-48 bg-slate-950 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between pointer-events-none">
          {gear.category?.name && (
            <Badge variant="primary" className="backdrop-blur-md bg-slate-950/80">
              {gear.category.name}
            </Badge>
          )}

          {gear.isAvailable ? (
            <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 backdrop-blur-md">
              <CheckCircle2 className="w-3 h-3 text-emerald-400" /> Available
            </span>
          ) : (
            <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full bg-slate-800/80 text-slate-400 border border-slate-700 backdrop-blur-md">
        ...
        <div className="mt-auto pt-3 border-t border-slate-800/80 flex items-center justify-between">
          <div>
            ...
            <div className="flex items-baseline gap-1">
              <span className="text-lg font-black text-slate-100">${gear.dailyRate}</span>
