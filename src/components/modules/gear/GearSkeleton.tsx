  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
      {Array.from({ length: count }).map((_, idx) => (
        <Card key={idx} className="flex flex-col h-96 overflow-hidden animate-pulse border-slate-800">
          <div className="w-full h-48 bg-slate-800/60" />
          <div className="p-5 flex flex-col gap-3 flex-1">
            <div className="h-3 w-1/3 bg-slate-800/80 rounded-md" />
            <div className="h-5 w-3/4 bg-slate-800/80 rounded-md" />
            <div className="h-3 w-full bg-slate-800/50 rounded-md" />
